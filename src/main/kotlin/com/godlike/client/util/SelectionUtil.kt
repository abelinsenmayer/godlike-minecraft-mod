...
fun showSelectionPreview(client: Minecraft) {
    val player = client.player!!
    val anchors = ModEntityComponents.CURSOR_ANCHORS.get(player).getPositions()

    var targetPos : Vec3i = blockRaycastFromPlayer()
    ModEntityComponents.TARGET_POSITION.get(player).setPos(BlockPos(targetPos))
    ...
    when (anchors.size) {
        0 -> {
            ...
            ModEntityComponents.TARGET_POSITION.get(player).setPos(targetPos)
        }
        1 -> {
            ...
            if (ModEntityComponents.SELECTING_VERTICAL.get(player).getValue()) {
                val selectingFar = ModEntityComponents.SELECTING_FAR.get(player).getValue()
            ...
            ModEntityComponents.CURSOR_PREVIEWS.get(player).addAllPositions(plane.map { BlockPos(it) })
        }
        else -> {
            ...
            ModEntityComponents.CURSOR_PREVIEWS.get(player).clearPositions()
            ModEntityComponents.CURSOR_PREVIEWS.get(player).addAllPositions(volume)
        }
    }
}
