...
class BlockPosListComponent(private val provider : Any) : AutoSyncedComponent {
    ...
    override fun readFromNbt(tag: CompoundTag) {
        clearPositions()
        addAllPositions(tag.getLongArray(com.godlike.common.components.positions).map { BlockPos.of(it) })
    }

    override fun writeToNbt(tag: CompoundTag) {
        synchronized(positions) {
            positions.map { it.asLong() }.toLongArray().let {
                tag.putLongArray(com.godlike.common.components.positions, it)
            }
        }
    }
